...
[path.join(LIVING, '..', 'family-tree-app', 'data.js'), path.join(LIVING, 'memory-graph.js'), path.join(LIVING, 'personas.js'),
 ...
 path.join(LIVING, 'journey.js'), path.join(LIVING, 'reflection.js'), path.join(LIVING, 'ingestion.js'), path.join(LIVING, 'agents.js')]
...
const JOURNEY = ctx.CASON_JOURNEY, REFLECTION = ctx.CASON_REFLECTION, INGESTION = ctx.CASON_INGESTION, AGENTS = ctx.CASON_AGENTS;
...
if (!JOURNEY || !REFLECTION || !INGESTION || !AGENTS) { console.log('  ✗ modules did not initialize'); process.exit(1); }
...
ok('registry: the three newly-built agents are registered live', ['ingestion', 'journey', 'reflection'].every(function (id) { const a = AGENTS.byId(id); return a && a.status === 'live'; }));
